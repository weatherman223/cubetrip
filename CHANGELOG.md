...
- 351 tests across 24 files (was 326). New coverage: parse-failure taxonomy (consent page, malformed JSON, structural anomaly) vs genuine-empty, bracket-in-string payloads (with a structurally-valid control fixture), provider error propagation, fallbackUrl survival through all-empty and all-error comp searches, midnight-hour time encoding, venue-timezone late checks (US/EU/invalid-zone), `utcToVenueLocal` (DST, day boundary, midnight), and `shiftDate` identity. Full suite passes under three process timezones.
